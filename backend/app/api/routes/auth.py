...
def _issue_token(db: Session, user: User) -> AuthResponse:
    ...
    db.commit()
    return AuthResponse(access_token=token, role=user.role.value, user_id=user.id)
...
def activate_client(payload: ActivateClientRequest, db: Session = Depends(get_db)) -> AuthResponse:
    ...
    user = User(email=payload.email, password_hash=hash_password(payload.password), role=UserRole.CLIENT)
    db.add(user)
    db.flush()
